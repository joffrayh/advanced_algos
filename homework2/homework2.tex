...
{%
\centering
\textit{(i.e. number of degrees from $v$ to part A and part B respectively)}\par
}
...
\]
Since each crossing edge contributes to $d_C(v)$ for two vertices,
\[
\sum_{v \in V} d_C(v) = 2|C|
\]
Similarly, since each edge contributes to $d(v)$ for two vertices,
\[
\sum_{v \in V} d(v) = 2|E|
\]
Combining these expressions:
\[
\sum_{v \in V} d_C(v) =2|C| \ge \frac{1}{2} \sum_{v \in V} d(v) = \frac{1}{2} (2|E|) = |E|
\]
Therefore,
\[
|C| \ge \frac{|E|}{2}
